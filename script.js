...
const GB = {
  ...
  initAnimations() {
    ...
    // Brothers section: scroll-driven canvas frame sequence (cinematic-sites pattern).
    GB.initAboutCanvas();
  },
  ...
  initAboutCanvas() {
    ...
    if (!canvas) return;
    // Skip on narrow viewports (mobile shows a looping <video> via CSS instead).
    ...
    for (let i = 1; i <= TOTAL; i++) {
      const img = new Image();
      img.src = 'assets/about-frames/frame-' + String(i).padStart(4, '0') + '.jpg';
      img.onload = () => {
        loaded++;
